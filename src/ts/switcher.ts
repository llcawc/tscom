...
export default function switcher() {
  ...
  type Tmode = 'dark' | 'light' | 'auto'

  // записать тему в локальное хранилище
  const setStoredTheme = (theme: Tmode) => localStorage.setItem('color-mode', theme)
  // считать тему из локального хранилища
  const getStoredTheme = () => localStorage.getItem('color-mode') as Tmode | null
  ...
  document.querySelectorAll('[data-theme-value]').forEach((toggle) => {
    toggle.addEventListener('click', () => {
      const theme = (toggle.getAttribute('data-theme-value') as Tmode | undefined) ?? 'auto'
